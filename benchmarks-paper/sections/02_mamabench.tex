...
assembling such questions from seven existing, expert-validated sources and
...
demographics}. A question about an allergic reaction that happens to involve a
pregnant patient is \textsc{none} (the allergy is the concept); postpartum
depression is \textsc{maternal}. The classifier is \texttt{Qwen3.6-27B-FP8}, run
...
\texttt{oss\_eval} items with the much larger \texttt{Qwen3.5-397B-A17B-FP8}
agrees with the 27B on
...
the physician annotations released with HealthBench's meta-evaluation
set~\cite{arora2025healthbench}; our only role is to re-scope that set to this
domain---keeping the triples whose prompts the scope classifier
...
rubric judge in exactly this way~\cite{ren2026mamai}.
